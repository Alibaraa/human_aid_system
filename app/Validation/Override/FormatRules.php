...
/**
 * Override of CodeIgniter\Validation\FormatRules
 * This file replaces the original FormatRules class to fix the valid_ip deprecation warning
 * for PHP 8.1+ when $which parameter is null
 * 
 * This file is loaded BEFORE the vendor version due to Composer autoload order
 */
...
class FormatRules
{
     ...
     * 
     * FIXED: PHP 8.1+ deprecation warning when $which is null
     *
    ...
    public function valid_ip(?string $ip = null, ?string $which = null): bool
    {
        ...
        // This must be done BEFORE any strtolower() call to prevent deprecation warnings
        if ($which === null || $which === '' || (is_string($which) && trim($which) === '')) {
            $which = 'ipv4';
        }

        // Explicitly cast to string to ensure it's never null when passed to strtolower()
        $whichString = (string) $which;
        $whichLower = strtolower($whichString);
        ...
        return (bool) filter_var($ip, FILTER_VALIDATE_IP, $filterFlag) || (! ctype_print($ip) && (bool) filter_var(inet_ntop($ip), FILTER_VALIDATE_IP, $filterFlag));
    }
